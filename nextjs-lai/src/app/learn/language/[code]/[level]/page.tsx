...
export default async function LanguageLevelPage({ params }: Props) {
    const { code, level } = await params
    const lang = resolveLanguage(code)
    if (!lang) notFound()

    const setId = `${code}-${level}`
    const staticSet = getLanguageSet(setId)
    ...
    const ttsLang = getLanguageBcp47(code)
    const studyBase = `/learn/language/${code}/study`
            ...
            {imported && (
                <>
                        ...
                        <Card className="border-orange-500/30 bg-gradient-to-r from-orange-500/5 to-background">
                            <CardContent className="flex items-center justify-between p-4">
                                ...
                                <Button size="sm" asChild>
                                    <Link href={`${studyBase}?mode=flip&doc=${documentId}`}>
                    ...
                    <div className="flex flex-wrap gap-3">
                        <Button asChild>
                            <Link href={`${studyBase}?mode=flip&all=true&doc=${documentId}`}>
                        ...
                        <Button variant="outline" asChild>
                            <Link href={`${studyBase}?mode=type&doc=${documentId}`}>
                                        ...
                                        <CardContent className="p-5 space-y-4">
                                            ...
                                            {cat.unlocked && (
                                                <div className="flex flex-wrap gap-2">
                                                    {cat.newCount > 0 && (
                                                        <Button size="sm" asChild>
                                                            <Link href={`${studyBase}?mode=flip&doc=${documentId}&category=${categoryParam}&new=true`}>
                                                    ...
                                                    {cat.dueCount > 0 && (
                                                        <Button size="sm" variant={cat.newCount === 0 ? 'default' : 'outline'} asChild>
                                                            <Link href={`${studyBase}?mode=flip&doc=${documentId}&category=${categoryParam}`}>
                                                    ...
                                                    <Button size="sm" variant="outline" asChild>
                                                        <Link href={`${studyBase}?mode=type&doc=${documentId}&category=${categoryParam}`}>
