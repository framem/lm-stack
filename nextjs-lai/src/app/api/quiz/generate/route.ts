...
import { SCENARIOS } from '@/src/lib/conversation-scenarios-constants'
import { getGeneratedScenarios } from '@/src/data-access/generated-scenarios'
import type { Language } from '@/src/lib/conversation-scenarios-constants'

// POST /api/quiz/generate - Generate quiz with SSE progress
export async function POST(request: NextRequest) {
    try {
        const body = await request.json()
        const { documentId, documentIds: rawDocumentIds, questionCount = 5, questionTypes = ['singleChoice'], difficulty = 1, scenarioKey, language } = body
        const diffLevel = (Math.min(Math.max(Number(difficulty), 1), 3)) as 1 | 2 | 3

        // ── Scenario mode: scenarioKey provided instead of documentIds ──
        if (scenarioKey) {
            ...
            if (types.length === 0) {
                return Response.json({ error: 'Mindestens ein gültiger Fragetyp ist erforderlich.' }, { status: 400 })
            }
            const count = Math.min(Math.max(Number(questionCount), 1), 20)
            const scenarioLanguage: Language = (language as Language) ?? 'de'
            ...
            const stream = new ReadableStream({
                async start(controller) {
                    ...
                    try {
                        // Look up scenario — static list first, then DB-generated
                        const staticScenario = SCENARIOS.find(s => s.key === scenarioKey)
                        let scenarioData: { title: string; difficulty: string; description: string; systemPrompt: string; suggestions: string[] } | null = null

                        if (staticScenario) {
                            const t = staticScenario.translations[scenarioLanguage] ?? staticScenario.translations['de']
                            scenarioData = {
                                title: t.title,
                                difficulty: staticScenario.difficulty,
                                description: t.description,
                                systemPrompt: t.systemPrompt,
                                suggestions: t.suggestions,
                            }
                        } else {
                            // Try DB-generated scenarios
                            const generated = await getGeneratedScenarios({ language: scenarioLanguage })
                            const found = generated.find(s => s.key === scenarioKey)
                            if (found) {
                                scenarioData = {
                                    title: found.title,
                                    ...
                                    suggestions: found.suggestions as string[],
                                }
                            }
                        }

                        if (!scenarioData) {
                            send({ type: 'error', message: 'Szenario nicht gefunden.' })
                            controller.close()
                            return
                        }

                        // Build context text from scenario metadata
                        const contextText = [
                            `Szenario: ${scenarioData.title} (${scenarioData.difficulty})`,
                            `Beschreibung: ${scenarioData.description}`,
                            '',
                            'Kontext und Vokabular:',
                            scenarioData.systemPrompt,
                            '',
                            'Wichtige Phrasen:',
                            ...scenarioData.suggestions.map(s => `- ${s}`),
                        ].join('\n')
                        ...
                        const label = 'Konversationsszenario'
                        for (const type of types) {
                            const typeCount = distribution[type] || 0
                            if (typeCount === 0) continue

                            if (type === 'singleChoice') {
                                const qs = await generateSingleChoiceQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                ...
                                const qs = await generateMultipleChoiceQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                ...
                                const qs = await generateFreetextQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                ...
                                const qs = await generateTruefalseQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                ...
                                const qs = await generateClozeQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                    allQuestions.push({ questionText: `${q.questionText}\n\n${q.sentenceWithBlank}`, options: null, correctIndex: null, correctAnswer: q.correctAnswer, explanation: q.explanation, sourceSnippet: q.sourceSnippet, questionType: 'cloze', difficulty: diffLevel })
                                    send({ type: 'progress', generated: allQuestions.length, total: count })
                                }
                            } else if (type === 'fillInBlanks') {
                                const qs = await generateFillInBlanksQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                    allQuestions.push({ questionText: `${q.questionText}\n\n${q.sentenceWithBlanks}`, options: null, correctIndex: null, correctAnswer: JSON.stringify(q.correctAnswers), explanation: q.explanation, sourceSnippet: q.sourceSnippet, questionType: 'fillInBlanks', difficulty: diffLevel })
                                    send({ type: 'progress', generated: allQuestions.length, total: count })
                                }
                            } else if (type === 'conjugation') {
                                const qs = await generateConjugationQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                                ...
                                const qs = await generateSentenceOrderQuestions(contextText, typeCount, diffLevel, label)
                                for (const q of qs) {
                        ...
                        const quizTitle = `Quiz – ${scenarioData.title}`
                        const quiz = await createQuiz(quizTitle, { scenarioKey, scenarioLanguage })
                        ...
                        console.error('Quiz generation error (scenario):', error)
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    ...
                    const quiz = await createQuiz(quizTitle, { documentId: documentIds[0] })
